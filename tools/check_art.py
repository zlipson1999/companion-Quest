...
and that companion families are three different creatures, not one pose.
...
def main():
    ...
    problems = len(broken) + len(orphaned) + len(clones)
    if problems:
        print(f'\n{problems} problem(s) above.')
        if clones:
            print('  CLONE/SHORT means the family fails the baby/adolescent/adult bar.')
