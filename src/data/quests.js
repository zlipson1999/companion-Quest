// The Quest Ledger: healthy-habit quests picked up and turned in at the
// Quest Fitness reception desk, paid for with Quest Credits.
//
// A quest is BOUGHT (the one place besides the smoothie bar that credits are
// spent), completed by real recorded behavior, and turned in for its Token
// plus a reward matched to its category through the same {xp,bond,evo,heal}
// contract everything else uses. Tokens are collectible proof, never money:
// they cannot be spent, do not convert to credits, and never touch trail
...
// purchase — a quest never invents its own counters.
...
export const QUESTS = [
  ...
  // The flagship: every system, none of them demanding perfection.
  {
    ...
    price: MILES(3), days: 7, flagship: true,
...
function reqHave(req, active, state) {
  ...
  switch (req.kind) {
    ...
    case 'cardio': {
      const rides = Math.max(0, (state.stats.ridesDone || 0) - (base.rides || 0));
      const mi = Math.max(0, (state.stats.distanceMi || 0) - (base.miles || 0));
      return rides >= 1 || mi >= 0.5 ? 1 : 0;
    }
    case 'moduleLogs': {
    ...
    case 'checkins': {
      const days = (state.quests && state.quests.checkIns) || [];
      return days.filter((d) => d >= active.startedDay).length;
    }
    default:
      return 0;
  }
