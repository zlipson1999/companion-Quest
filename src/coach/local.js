...
const INTENTS = [
  ...
  },
  {
    ...
    answer: (f) => {
      ...
      if (!mi) return 'No distance logged in the last two weeks. Trails move only after you choose one outside. The gym bike and treadmill keep separate cardio records, but add no trail progress or Trail Credit.';
      const cycled = Math.round((f.recent.cyclingMi || 0) * 10) / 10;
      return `${mi} total miles over the last ${RECENT_DAYS} days${cycled ? `, including ${cycled} cycled across ${f.recent.rides || 0} rides` : ''}; ${Math.round(f.week.distanceMi * 10) / 10} miles this week. Only miles logged on a selected outdoor trail advance its quota, milestones, encounters, or Trail Credit. Gym bike and treadmill miles stay in cardio history instead.`;
    },
  },
];
