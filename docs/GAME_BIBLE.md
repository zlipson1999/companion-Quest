...
- **Real life is the game.** No walk buttons. Distance comes from the pedometer,
  ...
  (see §5.3, §7.4).
- **Handheld-era presentation.** Follow-camera overworld with map overview, pixel font
...
navigation or state library. Pixel art retains generated grid/palette source data, including converted traced
references (§8.4). Authored companion and character art also ships as two-frame PNG
sheets under `assets/creature-bitmaps/`, generated by `render_sprite_bitmaps.mjs`
from those same palettes and idle frames. This avoids fractional strip seams
at small display sizes. Sunkist Lane uses continuous scene art under
`assets/worlds/`, with the collision map kept separately in `maps.js`. Other
raster assets include the tile atlas under `assets/tiles/`,
...
- **Sunkist Lane is a continuous landscape.** `worldArt.js` selects the scene
...
- **Sprites snap to whole device pixels, and drop resolution when they must**
  ...
  **1.5** device pixels per sprite pixel it halves, never past 24 rows. Then
  ...
  for a barcode.
  Companion sizes went up to match: the strip draws at **56**, the roster and
