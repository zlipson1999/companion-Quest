...
export default function GuidedSessionScreen() {
  ...
  useEffect(() => { if (exercise && progress >= exercise.target) setRunning(false); }, [progress, exercise]);

  const label = useMemo(() => {
    if (!exercise) return '';
    return exercise.kind === 'timer' ? `${progress} / ${exercise.target} sec` : `${progress} / ${exercise.target} reps`;
  }, [exercise, progress]);

  const next = () => {
    playSfx('confirm');
    if (index >= EXERCISES.length - 1) return setPhase('complete');
    setIndex((i) => i + 1); setProgress(0); setRunning(false); setAlternate(false);
  };

  const finish = () => {
    // The gym only routes here before mapleSessionDone, and this guard also
    // protects a stale/direct route. Reward through existing reducer actions
    // so the session counts exactly like a Forge workout rather than a fake tutorial stat.
    if (!state.meta.mapleSessionDone) {
      dispatch({ type: 'MARK_META', payload: { mapleSessionDone: true, mapleSessionReady: false } });
      dispatch({ type: 'COMPLETE_WORKOUT', payload: { workoutId: 'maple_first_session', sets: 5, reward: { xp: 20, bond: 8 } } });
      for (let i = 0; i < 5; i += 1) dispatch({ type: 'COLLECT_ITEM', payload: { itemId: 'knot' } });
      dispatch({ type: 'COLLECT_ITEM', payload: { itemId: 'berryblend' } });
    }
    playSfx('heal');
    navigate('gym');
  };

  if (phase === 'intro') return <Screen style={{ padding: space.md, justifyContent: 'flex-end' }}><DialogueBox lines={INTRO} onComplete={() => setPhase('work')} /></Screen>;
  if (phase === 'complete') return <Screen style={{ padding: space.md, justifyContent: 'flex-end' }}><DialogueBox lines={COMPLETE} onComplete={finish} /></Screen>;

  return (
    <Screen style={{ padding: space.md }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: space.md }}>
        <View style={{ flex: 1 }}><PixelText size="tiny" color={palette.muted}>COACH MAPLE · FIRST SESSION</PixelText><PixelText size="small" color={palette.white}>{exercise.name}</PixelText></View>
        {companion && companion.creature ? <PixelSprite spriteKey={companion.creature.sprite} palette={companion.creature.palette} size={64} bob /> : null}
      </View>
      <View style={{ flex: 1, justifyContent: 'center', gap: space.md }}>
        ...
        <ProgressBar value={progress} max={exercise.target} />
        {exercise.kind === 'timer' ? <PixelButton label={running ? 'PAUSE' : progress ? 'RESUME' : 'READY — START'} onPress={() => setRunning((v) => !v)} /> : exercise.kind === 'alternate' ? (
          <View style={{ flexDirection: 'row', gap: space.sm }}><View style={{ flex: 1 }}><PixelButton label="LEFT" disabled={progress >= exercise.target} onPress={() => setProgress((p) => Math.min(exercise.target, p + 1))} /></View><View style={{ flex: 1 }}><PixelButton label="RIGHT" disabled={progress >= exercise.target} onPress={() => setProgress((p) => Math.min(exercise.target, p + 1))} /></View></View>
