...
export default function HomeRestScreen() {
  ...
  // The first sleep after the home lesson does not just end the day — the
  // phone rings the morning after. Maple calls you back for your first guided
  // session, and keeps calling each morning until you have actually come.
  const mapleCalls = !!(companionName && state.meta.homeTourDone && !state.meta.mapleSessionDone);
  const lines = useMemo(() => {
    ...
    if (mapleCalls) {
      out.push({ speaker: 'Home', text: 'Morning. Your phone buzzes on the nightstand.' });
      out.push({ speaker: 'Coach Maple', text: '“Good morning! Maple here. You have seen my whole floor and taken your first loss — now come do your first real session. Nothing heavy: five easy movements, I lead, you follow. Find me at Quest Fitness.”' });
      out.push({ speaker: companionName, text: 'A first session, together. Let’s go!' });
    }
    return out;
  }, [companionName, mapleCalls]);
